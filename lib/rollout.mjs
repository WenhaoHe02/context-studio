...
function buildEntries(document) {
  ...
  for (const record of document.records) {
    ...
    if (value.type === "response_item" && payload.type === "message") {
      ...
      if (parts.length) {
        ...
        entries.push({
          ...
          inActiveContext: afterCompaction && payload.role !== "system",
          editable,
          ...
          sourceLabel: afterCompaction ? "当前历史" : "compact 前归档",
    ...
    if (value.type === "response_item" && payload.type === "reasoning") {
      ...
      entries.push({
        ...
        archived: !afterCompaction, sourceLabel: afterCompaction ? "当前历史" : "compact 前归档", sortIndex: record.index * 100000,
      });
    }
  ...
  if (compactedRecord && Array.isArray(replacementHistory)) {
    entries.push(...buildReplacementHistoryEntries(compactedRecord.index, replacementHistory));
  }

  return entries.sort((a, b) => (a.sortIndex ?? a.lineIndex) - (b.sortIndex ?? b.lineIndex));
}

function buildReplacementHistoryEntries(lineIndex, history) {
  const entries = [];
  const calls = new Map();
  history.forEach((item, historyIndex) => {
    ...
    if (item?.type === "message") {
      ...
      entries.push({
        ...
        deletable: skillOnly || editable, deleteLineIndices: [], deleteHistoryIndices: [historyIndex],
        ...
        contentPartCount: parts.length, inActiveContext: true,
        archived: false, sourceLabel: "compact 有效快照",
      });
    }
    if (item?.type === "reasoning") {
      ...
      entries.push({
        ...
        editable: false, deletable: true, deleteLineIndices: [], deleteHistoryIndices: [historyIndex],
        lockedReason: "Compacted encrypted reasoning can only be removed as a whole",
        inActiveContext: true, archived: false, sourceLabel: "compact 有效快照",
      });
    }
    if (TOOL_OUTPUT_TYPES.has(item?.type) && item.call_id) {
      ...
      entries.push({
        ...
        deleteLineIndices: [], deleteHistoryIndices: call ? [call.historyIndex, historyIndex] : [],
...
export function applyPatches(document, patches) {
  ...
  for (const patch of patches) {
    ...
    if (entry.kind === "message") {
      ...
      if (entry.container !== "replacement_history") synchronizeDisplayEvents(document, entry, entry.originalText, patch.text);
