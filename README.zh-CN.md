...
- **按来源拆分上下文**：最新 replacement history、compact 后新增内容和 compact 前原始记录各自独立成栏，再按消息、工具、可修改、可删除和只读类型筛选。
- **上下文感知编辑**：允许修改用户/助手文本与匹配的字符串工具输出，结构字段始终锁定。
- **安全的整项删除**：删除最新 compaction 后已完成的 reasoning、成对工具/MCP 事务和纯 skill 片段。
- **Desktop 可见记录同步**：compact 快照中能够精确匹配的消息会同步修改或删除其原始展示记录，随后自动打开目标任务，让 Desktop 重建可见历史。
- **官方 token 校准**：把条目估算与 Codex `token_count` 事件结合。
- **缓存命中率可视化**：在侧栏直接查看官方最近请求与会话累计命中率，并分别显示缓存与非缓存输入 Token。
- **前缀复用影响预览**：实时判断改动是否只落在当前模型输入尾部，显示首个变化位置以及预计仍可复用的本地历史前缀 Token/比例。
...
嵌入式写入流程会先暂存修改，再让 Codex 归档并卸载目标，提交校验后的字节，解除归档并核对哈希，最后自动打开目标任务，让 Desktop 从修改后的 rollout 重建可见消息。
...
“前缀复用影响”按 replacement history 与 compaction 后缀的实际模型顺序逐项比较。修改最后一个条目或连续删除尾部条目会显示“仅修改尾部”；如果首个变化之后仍保留旧条目，则显示“中间前缀变化”。它是本地保守估算，不代表服务端缓存一定仍在，也不覆盖 JSONL 外部注入内容。
